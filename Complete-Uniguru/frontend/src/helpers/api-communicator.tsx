...
export const sendVoiceQuery = async (
  ...
  const response = await axios.post<VoiceQueryResponse>("/voice/query", audioBlob, {
    headers: {
      "Content-Type": audioBlob.type || "application/octet-stream",
      "X-Audio-Filename": filename,
      "X-Allow-Web": String(Boolean(options?.allowWeb)),
      ...(options?.caller ? { "X-Caller-Name": options.caller } : {}),
      ...(options?.sessionId ? { "X-Session-Id": options.sessionId } : {}),
      ...(options?.language ? { "X-Voice-Language": options.language } : {})
    },
  });
  return response.data;
};
